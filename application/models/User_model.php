...
class User_model extends CI_Model {
	...
	/**
	 * Facebook auth/reg function
	 */
	public function login_with_facebook($user) {
		...
		if (! isset ( $users ) || $users < 1) {
			...
			$register_id = $this->ion_auth->register ( $user ['email'], $password, $user ['email'], array (
			...
			if ($register_id) {
				...
				$this->ion_auth->login ( $user ['email'], $password, TRUE );
			}
		} else {
	...
	public function login($identity, $password, $remember = FALSE) {
		...
		$query = $this->db->select ( $this->identity_column . ', email,first_name, last_name, id, password, active, last_login' )->where ( $this->identity_column, $identity )->limit ( 1 )->order_by ( 'id', 'desc' )->get ( $this->tables ['users'] );
	 ...
	 * the configuration item recheck_timer.
	 * If it does, then it will check if the user is still active
	 *
	 * @return bool
	 */
	...
	public function login_remembered_user() {
		...
		// get the user
		$this->trigger_events ( 'extra_where' );
		$query = $this->db->select ( $this->identity_column . ', id, email,first_name,last_name, last_login' )->where ( $this->identity_column, urldecode ( get_cookie ( $this->config->item ( 'identity_cookie_name', 'ion_auth' ) ) ) )->where ( 'remember_code', get_cookie ( $this->config->item ( 'remember_cookie_name', 'ion_auth' ) ) )->where ( 'active', 1 )->limit ( 1 )->order_by ( 'id', 'desc' )->get ( $this->tables ['users'] );
